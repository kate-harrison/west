...
class Region(object):
    ...
    def get_protected_entities_of_type(self, protected_entities_type,
                                       use_fallthrough_if_not_found=False):
        """
        Retrieves the ProtectedEntities object of the specified type from
        this Region.
        ...
        :param protected_entities_type: the type of protected entity (e.g. \
          ...
          Note that the base class should be used rather than a particular \
          subclass.
        :type protected_entities_type: class object
        :param use_fallthrough_if_not_found: if True, returns a \
                :class:`ProtectedEntitiesDummy` if the entity type is not \
                found. If False, a TypeError will be raised instead.
        :return:
        """
    ...
    def get_portable_tvws_channel_list(self):
        """Returns a list of channels which are considered TVWS for portable
        devices."""
        return
    ...
    def get_frequency_bounds(self, channel):
        """Get the (low, high) frequency bounds (in MHz) for the specified
        channel. Returns (None, None) if the channel is not supported."""
        return
    ...
    def replace_protected_entities(self, protected_entities_type, new_protected_entities):
        """
        Raises a TypeError if the entity type is not protected by the Region
        or if ``new_protected_entities`` is not an instance of a subclass of
        ``protected_entities_type``.
        ...
        :param protected_entities_type: the type of protected entity (e.g. \
          ...
          subclass.
        :type protected_entities_type: class object
        :param new_protected_entities: object containing the protected \
          entities to be used
        :type new_protected_entities: \
          :class:`protected_entities.ProtectedEntities`
        """
        ...
        if not isinstance(new_protected_entities, protected_entities_type):
            raise TypeError("New ProtectedEntities object (%s) is not "
                            "actually of the declared type (%s)."
                            % (str(new_protected_entities),
                               protected_entities_type.__name__))
