...
class PropagationCurve(object):
    """
    This class is a poor-man's enum because Python 2 does not support the
    Enum class.
    """
    ...
    @property
    def curve_50_10(cls):
        """Field strength exceeded at 50% of the locations at least 10% of
        the time."""
        return 0
    ...
    @property
    def curve_50_50(cls):
        """Field strength exceeded at 50% of the locations at least 50% of
        the time."""
        return 0
    ...
    @property
    def curve_50_90(cls):
        """Field strength exceeded at 50% of the locations at least 90% of
        the time."""
        return 0
...
class PropagationModel(object):
    ...
    def get_pathloss_coefficient(self, distance, frequency=None, tx_height=None,
                                 rx_height=None, tx_location=None,
                                 rx_location=None, curve_enum=None):
        """
        Computes the pathloss coefficient given a distance.

        Identical to :meth:`get_pathloss_coefficient_unchecked` but includes
        input validation.

          * For rare computations, use :meth:`get_pathloss_coefficient` alone.
          * For repeated computations where efficiency is key,
            use :meth:`parameters_are_sufficient` and
        ...
        :rtype: float
        """
        has_all_params, error_message = self._has_required_parameters(frequency,
                                                                      tx_height,
                                                                      rx_height,
                                                                      tx_location,
                                                                      rx_location,
        ...
        if not has_all_params:
            raise AttributeError(
                "Missing one or more required parameters: %s" % error_message)

        self._raise_error_if_input_invalid(frequency, tx_height, rx_height,
    ...
    def get_pathloss_coefficient_unchecked(self, distance, frequency=None,
        ...
        Identical to :meth:`get_pathloss_coefficient` but skips input
        validation (including the check for necessary parameters). However,
        this will increase efficiency in cases where the function is called
        repeatedly. In that case, it is recommended to use
        :meth:`parameters_are_sufficient` first to ensure that the necessary
    ...
    def get_distance(self, pathloss_coefficient, frequency=None, tx_height=None,
                     rx_height=None, tx_location=None,
                     rx_location=None, curve_enum=None):
        """
        Given a pathloss coefficient, returns the distance.

        Identical to :meth:`get_distance_unchecked` but includes input
        validation.

          * For rare computations, use :meth:`get_distance` alone.
          * For repeated computations where efficiency is key, use
            :meth:`parameters_are_sufficient` and
            :meth:`get_distance_unchecked`.

        :param pathloss_coefficient: pathloss coefficient (e.g. from \
                :meth:`get_pathloss_coefficient`)
        ...
        return self.get_distance_unchecked(
            pathloss_coefficient=pathloss_coefficient,
    ...
    def get_distance_unchecked(self, pathloss_coefficient, frequency=None,
        ...
        Identical to :meth:`get_distance` but skips input validation (
        including the check for necessary parameters). However, this will
        increase efficiency in cases where the function is called repeatedly. In
        that case, it is recommended to use :meth:`parameters_are_sufficient`
        first to ensure that the necessary parameters will be provided.
    ...
    def parameters_are_sufficient(self, provides_terrain=False,
                                  provides_tx_height=False,
                                  provides_rx_height=False,
                                  provides_tx_location=False,
                                  provides_rx_location=False,
                                  provides_frequency=False,
                                  provides_curve_enum=False):
        """
        Checks to see if the provided parameters are sufficient for use with
        this propagation model. This function is intended as a pre-check by
        algorithms which will use :meth:`get_pathloss_coefficient_unchecked`
        or :meth:`get_distance_unchecked`, as a balance between catching
        errors and efficiency.

        Each parameter is boolean, specifying whether or not the algorithm
        intends to specify the corresponding parameter when calling
        :meth:`get_pathloss_coefficient_unchecked`.
    ...
    def _has_required_parameters(self, frequency, tx_height, rx_height,
                                 tx_location, rx_location, curve_enum):
        """
        Checks to see if required input parameters have been specified. Does
        not check values or types. Checks that the length of a location
        parameter (if required) is exactly 2.
        ...
        if self.requires_tx_location() and (
                tx_location is None or len(tx_location) is not 2):
            return False, "missing or invalid transmitter location"

        if self.requires_rx_location() and (
                rx_location is None or len(rx_location) is not 2):
    ...
    def _raise_error_if_input_invalid(self, frequency, tx_height, rx_height,
        ...
        """
        This function is meant to be overridden by derived classes if
        customized behavior is desired. This documentation describes its
        intended behavior but by default does nothing.

        Checks to see if the required input parameters are in a valid range.
        Raises ValueError.
        """
        return
