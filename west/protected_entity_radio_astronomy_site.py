...
class ProtectedEntityRadioAstronomySite(ProtectedEntity):
    ...
    def __init__(self, container, region, latitude, longitude, channel, name,
                 is_point, latitude_deviation=None, longitude_deviation=None):
        super(ProtectedEntityRadioAstronomySite, self).__init__(region,
                                                                container,
                                                                latitude,
                                                                longitude)
        ...
        if not is_point and (latitude_deviation is None or
                                     longitude_deviation is None):
            self.log.error("Non-point radio astronomy site specified but "
                           "missing a latitude/longitude deviation value.")
    ...
    def to_string(self):
        output = "%s\tLatitude: %.2f\t\tLongitude: %.2f" % (str(
            self._name).ljust(30), self._latitude, self._longitude)
        if self._is_point:
            output += "\t(point)"
        else:
            output += "\t(rectangle; deviations: %.2f, %.2f)" % (
                self._latitude_deviation, self._longitude_deviation)
        return output
    ...
    def add_to_kml(self, kml):
        if self.is_point():
            ...
            Latitude: %.2f
            Longitude: %.2f
            """ % (self._name, self.get_channel(), self.get_latitude(),
                   self.get_longitude())
            ...
            lonlat_coordinates = [ (lon, lat) for (lat, lon) in
                                   latlon_coordinates]
            ...
            """ % (self._name, self.get_channel(), self.get_latitude(),
                   self._latitude_deviation, self.get_longitude(),
                   self._longitude_deviation)
            return poly

    def is_point(self):
        """
        Some RAS sites are points and others are polygons. This function
        differentiates them.
    ...
    def _create_new_bounding_box(self):
        """
        Creates an alternative bounding box for non-point entries. Note that
        in this case the bounding box is the same as the protected region
        (both rectangles).
    ...
    def location_in_protected_polygon(self, device_location):
        if self.is_point():
            ...
            return (
                (self._latitude - self._latitude_deviation <= lat <=
                 self._latitude + self._latitude_deviation)
                and
                (self._longitude - self._longitude_deviation <= lon <=
                 self._longitude + self._longitude_deviation)
            )
