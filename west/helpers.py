...
def channels_are_adjacent_in_frequency(region, chan1, chan2):
    try:
        ...
        # If the channel is undefined for the region, it will return a
        # ValueError. In that case, the channels are defined as not being
        # adjacent.
        return False
...
def add_bounding_box_to_kml(kml, bounding_box):
    ...
    poly.outerboundaryis.coords = [
        ...
        (bounding_box['max_lon'], bounding_box['max_lat'])
        # repeat the last one for closure
    ]
...
def lists_are_almost_equal(array1, array2, tolerance=1e-4):
    """
    Returns True if corresponding elements differ by no more than the
    tolerance. Returns False otherwise.
    """
    return all([abs(element1 - element2) < tolerance for element1, element2 in
                zip(array1, array2)])
...
def find_first_index_where(list_to_search, is_found_function):
    ...
    :param is_found_function: function which operates on a single element and \
            outputs True if the desired condition is met
    ...
    try:
        first_index = next(
            index for index, value in enumerate(list_to_search) if
            is_found_function(value))
...
def find_first_value_above_or_equal(list_to_search, value_to_find):
    """
    Returns the first index where the ``list_to_search[index] >=
    value_to_find``. Returns None if no such index exists.
    """
...
def find_last_value_below_or_equal(list_to_search, value_to_find):
    """
    Returns the last index where the ``list_to_search[index] <=
    value_to_find``. Returns None if no such index exists.
    """
...
def find_first_value_approximately_equal(list_to_search, value_to_find,
                                         tolerance=1e-4):
    """
    Returns the first index such that
    ``abs(list_to_search[index] - value_to_find) < tolerance``. Returns None
    if no such index exists.
    """
    return find_first_index_where(list_to_search,
                                  lambda v: abs(value_to_find - v) < tolerance)
