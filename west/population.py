...
class PopulationNugget(object):
    ...
    def to_string(self):
        return "Has geometry: %s\nPopulation: %s" % (str(self._geometry is not None), str(self._population))
...
class PopulationData(object):
    ...
    def _prune_invalid_population_nuggets(self):
        num_nuggets_before_pruning = len(self._population_nuggets_by_identifier.keys())
        self._population_nuggets_by_identifier = {identifier: nugget
                                                  for
                                                  identifier, nugget in self._population_nuggets_by_identifier.items()
                                                  if nugget.is_valid()}
        num_nuggets_after_pruning = len(self._population_nuggets_by_identifier.keys())

        if num_nuggets_after_pruning == num_nuggets_before_pruning:
            self.log.debug("No population nuggets were pruned.")
        else:
            self.log.debug("%d population nuggets were pruned due to insufficient data (%d remaining)" %
                           (num_nuggets_before_pruning - num_nuggets_after_pruning, num_nuggets_after_pruning))
    ...
    def create_population_map(self, is_in_region_datamap2d):
        ...
        population_datamap2d = data_map.DataMap2D.get_copy_of(is_in_region_datamap2d)
        population_datamap2d.reset_all_values(0)    # set everything to 0 so we can accumulate

        latitude_width_degrees = self._calculate_latitude_width_degrees(is_in_region_datamap2d.latitudes)
        longitude_width_degrees = self._calculate_longitude_width_degrees(is_in_region_datamap2d.longitudes)

        def calculate_population(latitude, longitude, latitude_index, longitude_index, current_value):
            if not is_in_region_datamap2d.get_value_by_index(latitude_index, longitude_index):
                return None

            return self._calculate_population_for_pixel((latitude, longitude), latitude_width_degrees, longitude_width_degrees)

        # for (lat_idx, lat) in enumerate(population_datamap2d.latitudes):
        #     if lat_idx % 10 == 0:
        #         print "Latitude number: %d" % lat_idx
        #     for (lon_idx, lon) in enumerate(population_datamap2d.longitudes):
        #         # Skip if not in region
        #         if not is_in_region_datamap2d.mutable_matrix[lat_idx, lon_idx]:
        #             continue
        #
        #         population_datamap2d.mutable_matrix[lat_idx, lon_idx] = \
        #             self._calculate_population_for_pixel((lat, lon), latitude_width_degrees, longitude_width_degrees)

        population_datamap2d.update_all_values_via_function(calculate_population, verbose=True)

        return population_datamap2d

    def _calculate_population_for_pixel(self, location, latitude_width_degrees, longitude_width_degrees):
        current_pop = 0
        pixel_polygon = self._create_pixel_polygon(location, latitude_width_degrees, longitude_width_degrees)
        for population_nugget in self._population_nuggets_by_identifier.values():
            current_pop += self._get_population_contribution(pixel_polygon, population_nugget)
        return current_pop
    ...
    def _get_population_contribution(self, pixel_polygon, population_nugget):
        ...
        intersection = pixel_polygon.intersection(population_nugget.get_geometry())
        intersection_area = intersection.area
        nugget_area = population_nugget.get_geometry().area

        # Note: shapely does not know that these are geo coordinates; the area here is an approximation
    ...
    def _create_pixel_polygon(self, location, latitude_width_degrees, longitude_width_degrees):
        ...
        exterior = [(min_lon, max_lat), (max_lon, max_lat), (max_lon, min_lat), (min_lon, min_lat)]
        return shapely.geometry.Polygon(exterior)


class ShapefilePopulationData(PopulationData):
    """ Provide some methods that make it easier with shapefiles.
    """
...
class PopulationDataUnitedStates2010(ShapefilePopulationData):
    ...
    def geography_source_filenames(self):
        base_directory = os.path.join(configuration.paths['UnitedStates']['population'], str(self.data_year()))
        filename_list = []
        for fips_code in self._fips_code_list():
            base_filename = "gz_2010_%02d_140_00_500k" % fips_code
            # For example, Population/2010/gz_2010_01_140_00_500k/gz_2010_01_140_00_500k.shp
            filename_list.append(os.path.join(base_directory, base_filename, base_filename + ".shp"))

        return filename_list

    def population_source_filenames(self):
        return [os.path.join(configuration.paths['UnitedStates']['population'], str(self.data_year()),
                             "popbytract2010.csv"), ]

    def geography_source_name(self):
        return "US census bureau (http://www2.census.gov/geo/tiger/GENZ2010/ or " + \
    ...
    def _load_geography(self):
        ...
        for (geometry, properties) in geometries_and_properties:
            ...
            if identifier not in self._population_nuggets_by_identifier:
                self._population_nuggets_by_identifier[identifier] = PopulationNugget(identifier)

            self._population_nuggets_by_identifier[identifier].add_geometry(geometry)
    ...
    def _load_populations(self):
        ...
        with open(population_filename, "r") as f:
            ...
            for row in population_csv:
                ...
                self._population_nuggets_by_identifier[identifier].add_population(population)
